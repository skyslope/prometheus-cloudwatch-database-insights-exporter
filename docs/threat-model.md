...
Additional Go packages requiring AWS, Prometheus, or SQL dependencies could not build: the dependencies were not available, and attempts to create module-cache entries were denied. The command used `GOPROXY=off`, `GOSUMDB=off`, and `-mod=readonly`; dependencies were not downloaded and no permission escalation was attempted. HTTP/AWS-client integration tests, database connections, race detection, image builds, actual endpoint exposure, and diagram rendering were not performed. These limitations do not block this evidence-based document; they do limit runtime validation. No model refusal occurred.

The completed document was reread. `git diff --check -- docs/threat-model.md` and an additional untracked-file whitespace check reported no whitespace errors. This pull request adds `AGENTS.md`, `CLAUDE.md`, and `docs/threat-model.md`, with no pre-existing dirty files to preserve.
